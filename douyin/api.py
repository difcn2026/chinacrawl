...
⚠️ 2026-06 经验更新:
  - /aweme/v1/web/aweme/post/ 等端点已改用 protobuf 二进制格式
  - /aweme/v1/web/aweme/detail/ 同样返回二进制数据
  - 直接 HTTP API 调用不再可靠，建议优先使用 browser 模式获取用户作品
  - 详见 _parse_response() 中的 DouyinBinaryResponseError 处理
"""
...
def _parse_response(resp: httpx.Response) -> dict:
    ...
    ⚠️ 2026-06 经验补充:
       Douyin 的部分 API 端点（aweme/post/, aweme/detail/）已改用
       protobuf 二进制格式，不再返回 JSON。检测到二进制响应时抛出
       DouyinBinaryResponseError，调用方应降级到 browser 模式。

       检测逻辑: 响应内容不是 JSON 也不是 HTML，且大于 100 字节。
    """
    try:
        data = resp.json()
        ...
        raw = resp.content
        is_binary = (
            not raw[:10].strip().startswith(b'{')
            and not raw[:4].startswith(b'<!')
            and len(raw) > 100
        )
        if is_binary:
            raise DouyinBinaryResponseError(
                code=-2,
                message=f"Binary response ({len(raw)} bytes, content-type: {resp.headers.get('content-type','')}) - API may use protobuf",
                url=str(resp.url),
...
class DouyinBinaryResponseError(DouyinAPIError):
    """抖音 API 返回二进制数据（protobuf），需降级到 browser 模式"""
    def __init__(self, code: int, message: str, url: str = "",
                 status: int = 0, raw: dict = None, raw_size: int = 0):
        self.raw_size = raw_size
        super().__init__(code, message, url, status, raw)
...
def fetch_user_posts(sec_uid: str, max_cursor: int = 0,
    ...
    ⚠️ 2026-06: 此 API 可能返回二进制 protobuf 格式而非 JSON。
    遇到 DouyinBinaryResponseError 时，请使用 browser 模式替代：
        from chinacrawl.douyin.scraper import browser_fetch_user_posts
        videos = browser_fetch_user_posts(sec_uid)
    ...
    Returns:
        { "aweme_list": [...], "max_cursor": ..., "has_more": 1 }
    """
...
def fetch_user_posts_all(sec_uid: str) -> Generator[dict, None, None]:
    ...
    while True:
        ...
        except DouyinBinaryResponseError:
            log.error("Page %d: API returned binary data. Use browser_fetch_user_posts() instead.", page)
            break
        except DouyinAPIError:
