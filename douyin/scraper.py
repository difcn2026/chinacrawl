...
# ============================================================
# Browser Fallback: Video ID Extraction & Download (2026-06经验)
# ============================================================
#
# 背景: Douyin 的 /aweme/v1/web/aweme/post/ API 已改用 protobuf 二进制格式,
# 直接 HTTP 调用无法解析。但这不影响浏览器端的正常渲染。
# 以下方法绕过 API 限制，直接从浏览器获取视频。
#
# 策略: 打开用户主页 - 从 HTML 提取视频 ID - 逐个打开视频详情页
#       拦截 douyinvod 下载链接 - 下载
#
# 注意: 仅能获取 SSR 渲染的首屏视频 (通常约30个),
#       懒加载更多内容因 API 二进制格式而无法触发。
#


def browser_extract_video_ids(sec_uid: str, cookie_file: Optional[str] = None) -> list:
    """
    通过 Playwright 打开用户主页，从 HTML 中提取所有可见视频 ID。

    使用正则匹配 /video/(数字) 模式，不依赖 JavaScript 解析。

    Args:
        sec_uid: 用户加密 ID
        cookie_file: Cookie 文件路径 (可选，公开主页不需要)

    Returns:
        ["7495303724068277558", "7495966788396731684", ...]
    """
    from . import browser as br

    log.info("Opening user page to extract video IDs: %s...", sec_uid[:30])
    b = br.launch_browser(headless=False)
    ctx = br._create_context(b, cookie_file)
    p = ctx.new_page()
    try:
        p.goto(f"https://www.douyin.com/user/{sec_uid}",
               wait_until="domcontentloaded", timeout=60000)
        time.sleep(5)
        html = p.content()
        # Match /video/ followed by 17+ digits
        ids = list(set(re.findall(r'/video/(\d{17,})', html)))
        log.info("Found %d video IDs", len(ids))
        return ids
    finally:
        p.close()
        ctx.close()
...
def browser_download_video(aweme_id: str, output_path: str,
                            cookie_file: Optional[str] = None) -> bool:
    """
    通过 Playwright 打开视频详情页，拦截 douyinvod 响应获取下载链接并下载。

    适用于 2026-06 抖音网页版：API 返回二进制 protobuf,
    但视频详情页的 HTML 渲染正常，douyinvod CDN 链接仍然可用。

    Args:
        aweme_id: 视频 ID
        output_path: 保存路径 (如 ./downloads/v_7495303724068277558.mp4)
    ...
    def _capture(resp):
        if 'douyinvod' in resp.url:
            dl_urls.append(resp.url)

    b = br.launch_browser(headless=False)
    ctx = br._create_context(b, cookie_file)
    p = ctx.new_page()
    p.on('response', _capture)

    try:
        p.goto(f"https://www.douyin.com/video/{aweme_id}",
               wait_until="domcontentloaded", timeout=30000)
        time.sleep(3)

        # 如果没拦截到 douyinvod，尝试从 video 标签取 src
        if not dl_urls:
            src = p.evaluate("""() => {
                const v = document.querySelector('video');
                return v && v.src && !v.src.startsWith('blob:') ? v.src : '';
            }""")
            if src:
                dl_urls.append(src)

        if not dl_urls:
            log.warning("No download URL found for video %s", aweme_id)
            return False

        url = dl_urls[0]
        resp = httpx.get(url, headers={"User-Agent": "Mozilla/5.0", "Referer": "https://www.douyin.com/"},
                          timeout=60, follow_redirects=True)
        if resp.status_code == 200 and len(resp.content) > 10000:
            os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
            with open(output_path, "wb") as f:
                f.write(resp.content)
            log.info("Downloaded: %s (%.1f MB)", output_path, len(resp.content) / 1024 / 1024)
            return True
        else:
            log.warning("Download failed: HTTP %d, %d bytes", resp.status_code, len(resp.content))
            return False

    except Exception as e:
        log.warning("Failed to download video %s: %s", aweme_id, e)
        return False
    finally:
        p.close()
        ctx.close()
